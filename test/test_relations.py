...
class relationsTest(unittest.TestCase):
    ...
    def test_attribute_nature_complex(self):
        ...
        self.assertEqual(d["relations"][4]["columns"][1]["nature"], u"promoting_foreign_key")
        self.assertEqual(d["relations"][4]["columns"][1]["label"], u"fail")
    ...
    def test_demoted_foreign_key(self):
        ...
        self.assertEqual(d["relations"][1]["columns"][1]["nature"], u"foreign_primary_key")
        self.assertEqual(d["relations"][1]["columns"][1]["primary_relation_name"], u"TELLUS")
        self.assertEqual(d["relations"][1]["columns"][2]["nature"], u"demoted_foreign_key")
        self.assertEqual(d["relations"][1]["columns"][2]["primary_relation_name"], u"EROS")
    ...
    def test_inheritance_leftwards_double_arrow(self):
        ...
        """
        text = """
            ANIMAL (_animal_, CARNIVORE, quantité viande, HERBIVORE, plante préférée)
        """.strip().replace("    ", "")
        t = Relations(Mcd(clauses.split("\n"), params), params)
        self.assertEqual(t.get_text(minimal_template), text)
        d = json.loads(t.get_text(json_template))
    ...
    def test_inheritance_leftwards_simple_arrow(self):
        ...
        """
        text = """
            ANIMAL (_animal_, type, quantité viande, plante préférée)
        """.strip().replace("    ", "")
        t = Relations(Mcd(clauses.split("\n"), params), params)
        self.assertEqual(t.get_text(minimal_template), text)
        d = json.loads(t.get_text(json_template))
