...
CREATE TABLE exemplaire (
  ...
  FOREIGN KEY (cote_oeuvre) REFERENCES oeuvre (cote_oeuvre)
);
...
CREATE TABLE reserver1 (
  ...
  FOREIGN KEY (numero) REFERENCES chambre1 (numero)
);
```
...
CREATE TABLE reserver2 (
  ...
  id_client VARCHAR(8),
  PRIMARY KEY (numero, date)
  FOREIGN KEY (numero) REFERENCES chambre2 (numero)
);
```
...
CREATE TABLE ligula1 (
  ...
  metus VARCHAR(42),
  PRIMARY KEY (blandit)
  FOREIGN KEY (blandit) REFERENCES lacus1 (blandit),
  FOREIGN KEY (congue) REFERENCES eros1 (congue)
);
```
...
CREATE UNIQUE INDEX client3_u1 ON client3 (nom, prenom);
CREATE UNIQUE INDEX client3_u2 ON client3 (mail);
```
...
CREATE TABLE lacus (
  ...
  FOREIGN KEY (integer) REFERENCES tellus (integer)
);
...
CREATE TABLE tellus (
  ...
  ipsum VARCHAR(42),
  PRIMARY KEY (integer)
  FOREIGN KEY (integer_1) REFERENCES tellus (integer),
  FOREIGN KEY (congue) REFERENCES eros (congue)
);
```
...
CREATE TABLE etudiant (
  ...
  FOREIGN KEY (num_enseignant_responsable) REFERENCES enseignant (num_enseignant)
);
```
...
CREATE TABLE commande1 (
  ...
  PRIMARY KEY (num_commande)
  FOREIGN KEY (ref_client_ayant_commande) REFERENCES client4 (ref_client)
);
...
CREATE TABLE inclure1 (
  ...
  FOREIGN KEY (num_commande_passee) REFERENCES commande1 (num_commande),
  FOREIGN KEY (ref_produit_commande) REFERENCES produit1 (ref_produit)
);
