...
def main(mcd, common):
    ...
    commands = {
        ...
        "straight_card_note": """(tx,ty)=leg.card_pos(%(cw)s+2*card_margin,%(ch)s+2*card_margin,k[u"%(leg_identifier)s"])\nfill(colors["%(text_color)s"])\nfont("%(family)s",%(size)s)\ntext(u"%(text)s",tx,ty)""",
        "straight_arrow": """arrow(*leg.arrow_pos("%(direction)s",t[u"%(leg_identifier)s"]))""",
        ...
        "curved_card_note": """(tx,ty)=leg.card_pos(%(cw)s+2*card_margin,%(ch)s+2*card_margin,k[u"%(leg_identifier)s"])\nfill(colors["%(text_color)s"])\nfont("%(family)s",%(size)s)\ntext(u"%(text)s",tx,ty)""",
        "curved_arrow": """arrow(*leg.arrow_pos("%(direction)s",t[u"%(leg_identifier)s"]))""",
        "card_underline": """line(tx,ty-card_underline_skip_height,tx+%(w)s,ty-card_underline_skip_height)""",
    }
